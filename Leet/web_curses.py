"""
Minimal in-memory curses shim for running leet.py inside Pyodide.
...
curses API leet.py touches: a screen buffer with addch/addstr/erase/
refresh/getmaxyx, a tiny color-pair table, the handful of key and
mouse-button constants leet.py compares against, and a non-blocking
getch()/getmouse() pair fed by push_key()/push_mouse() from the JS side.
There is no real device underneath - the browser renders whatever's in
the buffer (via Screen.render_rows()) and forwards DOM events into the
input queue.

leet.py never imports this module by name. The web bridge script
registers an instance of it into sys.modules['curses'] before leet.py
is loaded, so leet.py's own `import curses` resolves here instead of
to a real (absent) curses.

Unlike Terminal's RTS (box-select: only needs the press/release corners
of a drag), leet.py's flight controls need the *current* cursor position
every tick while a button is held, and - in its default "Auto Seek
Cursor" mouse mode - even with no button held at all. Pushing one queued
event per real mousemove would let the queue outrun leet.py's own tick
rate under fast mouse movement, since Screen.getch() only ever dequeues
one entry per tick. push_mouse() below coalesces consecutive, still-
unconsumed pure-motion reports into the latest one instead of piling
up a backlog; a press or release is never coalesced away, so those
discrete transitions always still arrive.
"""
...
# ---- key constants: arbitrary but distinct. leet.py only ever compares
...
def wrapper(fn):
    ...
    ever gets exec'd directly instead of imported as 'leet'."""
    return fn(Screen())
...
class Screen:
    ...
    # -------- curses-alike surface used by leet.py --------
    ...
    # -------- driven by the JS bridge, not by leet.py --------
    ...
    def push_mouse(self, x, y, bstate):
        event = ("mouse", (0, x, y, 0, bstate))
        if bstate == REPORT_MOUSE_POSITION and self._input and self._input[-1][0] == "mouse" \
                and self._input[-1][1][4] == REPORT_MOUSE_POSITION:
            # Coalesce: this is a pure-motion report and the queue's tail is
            # still an unconsumed pure-motion report - replace it instead of
            # growing the queue. A press or release always still lands as
            # its own entry (the bstate check above only matches motion).
            self._input[-1] = event
        else:
