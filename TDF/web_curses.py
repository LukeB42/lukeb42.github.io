"""
Minimal in-memory curses shim for running tdf.py inside Pyodide.
...
curses API tdf.py touches: a screen buffer with addch/addstr/
erase/refresh/getmaxyx, a color-pair table (basic 8 colors plus the
xterm-256 cube/grayscale indices tdf.py's per-level palettes reach for
when curses.COLORS >= 256), the handful of key and mouse-button
constants tdf.py compares against, and a non-blocking getch()/getmouse()
pair fed by push_key()/push_mouse() from the JS side. There is no real
device underneath - the browser renders whatever's in the buffer (via
Screen.render_rows()) and forwards DOM events into the input queue.

tdf.py never imports this module by name. The web bridge script
registers an instance of it into sys.modules['curses'] before
tdf.py is loaded, so tdf.py's own `import curses` resolves here
instead of to a real (absent) curses.
"""
...
# tdf.py checks `curses.COLORS >= 256` (see apply_palette) to decide whether
# to reach for the richer xterm-256 shades instead of the basic 8. This shim
# resolves those indices for real (see _xterm256_hex below), so it can
# honestly claim the same 256-color support a real xterm would.
COLORS = 256
...
# Kept faithful to a plain terminal's ANSI palette (not skewed toward any
# one mission's palette) since the codec briefing - Iona/Selma/Gene - and
# a few other fixed-pair reads (e.g. default/uncolored text) always go
# through these 8 regardless of which level's 256-color palette is active. ----
_HEX = {
...
def _xterm256_hex(n):
    ...
    plus the grayscale ramp) - covers the extended blood-red / concrete /
    steel / hazard-amber shades tdf.py's per-level palettes use once
